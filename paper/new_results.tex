...
\paragraph{Extended orbit-weight sweep.} We extended the 2\% N64 Galilean \(\lambda_{\mathrm{orb}}\) sweep beyond the previous largest tested value, \(\lambda_{\mathrm{orb}}=0.10\). A one-seed probe at \(\lambda_{\mathrm{orb}}=0.20\) improved over the seed-23 \(\lambda_{\mathrm{orb}}=0.10\) run, so we expanded \(\lambda_{\mathrm{orb}}=0.30\) to three matched seeds. The higher weight improves all three paper-facing error metrics on these seeds, but it should be treated as a new promising setting rather than an immediately final headline until it is either expanded to five seeds or bracketed by a larger degrading value.

\begin{table}[t]
\centering
\small
\caption{Extended \(\lambda_{\mathrm{orb}}\) sweep at 2\% labels on N64 Galilean. Rows with five seeds are completed headline artifacts; high-\(\lambda\) rows are new probes.}
...
0.20 & \(0.3140\) & \(0.3368\) & \(0.2094\) & 1 \\
0.30 & \(\mathbf{0.3096\pm0.0086}\) & \(\mathbf{0.3326\pm0.0073}\) & \(\mathbf{0.1942\pm0.0070}\) & 3 \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Implication for the paper.} The new results strengthen the submission most by hardening the N64 Galilean story rather than by broadening scope. The core message should remain narrow: with the same FNO2d inference map, stochastic local orbit consistency improves symmetry-induced OOD error and equivariance defect, with the strongest benefits in low-label regimes and positive evidence through 10\% labels. The adaptation and semi-supervised runs should be presented as secondary evidence; they are valuable for reviewer questions about unlabeled data, but they are not stronger than the main supervised Aug+Orbit result.
