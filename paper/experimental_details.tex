...
\paragraph{Label-efficiency update schedules.}
The 1\%, 5\%, and 10\% comparisons contain 10, 51, and 102 labeled examples,
respectively, and use batch size 16 for 150 epochs.  At every fraction,
augmentation uses six optimizer updates per epoch (900 total), with two
batch-level model evaluations per update, whereas augmentation plus normalized
LOCO uses four updates per epoch (600 total), with three evaluations per update.
Thus both methods use 12 batch-level model evaluations per epoch and 1,800 over
training, although the numbers of optimizer updates and backward passes differ.
Accounting for retained partial batches, augmentation applies 9,000, 12,450, and
14,400 per-example transforms over training at 1\%, 5\%, and 10\%; augmentation
plus LOCO applies 6,000, 7,650, and 9,600 augmentation transforms and the same
respective numbers of independently drawn orbit transforms.  At 5\%, the fourth
batch contains only three examples, so the comparison matches batch-level but not
per-example model evaluations.
...
\addlinespace
rMD17 ethanol \citep{christensen2020rmd17,christensen2020rmd17data} & Fixed nine-atom molecule; 500/1,000/1,000 & Static force regression; no PDE solve & Released \texttt{rmd17\_ethanol.npz}; raw SHA-256 \texttt{0824d78c687b8ede25f7162c54c5c643}\allowbreak\texttt{4cfe37f9f859ee8f223c60379d7d5cf9}; historical custom selection with metadata seed 2027, rather than the official split CSVs & Subtract each conformation's arithmetic coordinate centroid; append nuclear charge \(Z/10\); targets are unnormalized forces. Exact cached 500-label tensor is preserved in the artifact with raw \texttt{old\_indices}; SHA-256 \texttt{fb5515b3355651459c2ad9078e7cfb5}\allowbreak\texttt{bb63dce96e55e7c36956cbe9ff25e5472}. Coordinates in \AA, forces in kcal mol\(^{-1}\)\AA\(^{-1}\) \\
\bottomrule
\end{tabular}%
}
\end{table*}
...
The reported Burgers comparison uses all 2,048 labeled training pairs.  Its orbit
metrics are not Galilean-only.  For each minibatch, we
...
\(0.35,0.525,0.70,1.05\).  For a translation draw the scalar step is
\(\epsilon=\max(|s|,10^{-6})\), and for a boost draw it is
\(\epsilon=\max(|b|,10^{-6})\); the normalized orbit term divides by
\(\epsilon^2+\eta\) with \(\eta=10^{-6}\).  Standard evaluation uses four draws
for each of 512 test examples (2,048 transformed example--draw pairs); the severity
analysis uses eight.

\paragraph{rMD17 source, split, and preprocessing.}
We use the fixed-order, nine-atom ethanol trajectory from the revised MD17 release
\citep{christensen2020rmd17,christensen2020rmd17data}.  For each conformation, we
subtract the arithmetic (not mass-weighted) coordinate centroid and append the
scaled nuclear charge to each atom:
\[
X_c=X-\mathbf 1\bar x^\top,\qquad a_i=(X_{c,i},Z_i/10),\qquad u_i=F_i.
\]
Coordinates are in \AA\ and force targets in kcal mol\(^{-1}\)\AA\(^{-1}\).
Inputs and targets are stored as \texttt{float32}; forces are otherwise
unnormalized, and energy labels are not used.  All optimization seeds
\(\{23,31,47,59,71\}\) reuse the same mutually disjoint 500/1,000/1,000
training/validation/test split.

This historical custom selection does not use the release's official split CSVs.
Its metadata records split seed 2027, but the historical sampler was not retained
and the current seeded \texttt{torch.randperm} does not reconstruct the selection.
The exact operational split rule is therefore to look up each split's stored
\texttt{old\_indices} in the raw NPZ's unique \texttt{old\_indices} array and
preserve the cached order.  The preserved tensor and its SHA-256 in
Table~\ref{tab:secondary-dataset-details} are the authoritative evidence input.
The released verifier maps all 2,500 identifiers back to the hashed NPZ with zero
\texttt{float32} input and force discrepancy.  The older 1,000-label cache has
different validation/test slices and is not used as a controlled label-scaling
comparison.

\paragraph{rMD17 rigid motions, losses, and evaluation.}
Each per-example transform draws
\(z\sim\mathcal N(0,I_3)\), sets
\(q=z/\max(\lVert z\rVert_2,10^{-8})\), draws
\(\theta\sim\mathcal U[-\pi/2,\pi/2]\), and draws translation components
\(t_j\stackrel{\mathrm{iid}}{\sim}\mathcal U[-1,1]\)~\AA.  If
\(Q(q,\theta)\) is the axis--angle rotation, then
\[
T_g^{\mathcal A}(X_c,Z)=(X_cQ^\top+\mathbf 1t^\top,Z),\qquad
T_g^{\mathcal U}F=FQ^\top.
\]
Coordinates are not recentered after this transformation.  The axis is uniform on
the sphere and the bounded angle is uniform on its interval; this is not a
Haar-uniform rotation draw.  The implemented composite step is
\(\epsilon=\max(|\theta|+\lVert t\rVert_2,10^{-6})\).  Because it combines
radians and \AA, it is a fixed disclosed weighting rather than a canonical,
unit-free \(\mathrm{SE}(3)\) norm.

The supervised and augmented terms use the minibatch mean of the unsquared
per-conformation relative force error,
\[
\ell_{\rm force}(\widehat F,F)
=\frac{\lVert\widehat F-F\rVert_F}{\max(\lVert F\rVert_F,10^{-8})},
\]
not force MAE or force MSE.  The molecular orbit term is the per-conformation mean
squared force-entry defect divided by \(\epsilon^2+10^{-3}\).  The reported
Aug.+LOCO row uses \(\lambda_{\rm aug}=1\) and
\(\lambda_{\rm orb}=0.03\); augmentation and LOCO draw independent rigid motions,
and gradients propagate through both prediction branches.  Force MAE is an
...
three evaluations per update.  Both therefore use 48 batch-level model evaluations
per epoch and 7,200 over 150 epochs.  Because 500 is not divisible by 32, this is
not exact at the example level: augmentation visits 756 examples and samples 756
rigid motions per epoch (113,400 over training), giving 1,512 sample-level model
evaluations per epoch, whereas Aug.+LOCO visits all 500 examples once and
independently samples one augmentation and one orbit motion for each (1,000 per
epoch; 150,000 total), giving 1,500 sample-level model evaluations per epoch.
Validation occurs every 25 epochs, and the
checkpoint is selected by untransformed validation relative force \(L^2\).  Final
evaluation averages four independent motions for each of the 1,000 test
conformations (4,000 conformation--motion pairs per run), with transform RNG seed
\(s+200{,}000\).  The evaluation ranges equal the training ranges, so these are
described as transformed-input rather than wider-severity OOD metrics.  We use 10
latency warm-ups and 50 repeats.
...
forward-pass compute is therefore not matched.  Evaluation is Monte Carlo rather
than an exhaustive average over all eight elements: for each of the 256 held-out
examples, it samples four independent uniform \(D_4\) elements with replacement,
so repetitions and the identity are possible, and averages the resulting 1,024
example--element pairs.  For optimization seed \(s\), the evaluation RNG is seeded
by \(s+200{,}000\), which matches the draws across methods with the same seed.
...
restore translation equivariance of the fixed-boundary heat equation.  Here
\(\epsilon=\max(|s|,10^{-6})\) and \(\eta=10^{-6}\).  The common-support
re-evaluation draws four shifts for each of 128 test examples, hence 512 transformed
example--draw pairs per checkpoint under each scoring support.  It uses evaluation
seed \(s_{\rm run}+700{,}000\); resetting that seed for every evaluation matches
the draws across all three methods and both scoring supports.
...
\(-T(\delta_x\partial_x^{\rm spec}+\delta_y\partial_y^{\rm spec})\mathcal G(a)\).
These are Fourier spectral derivatives, obtained by differentiating the same
Fourier phase-shift action used for the finite Galilean transform.  For
\(u\in\mathbb R^{H\times W}\) on \([0,L)^2\), the exact discrete implementation is
\[
\widehat u=\operatorname{FFT}_2(u),\qquad
\partial_x^{\rm spec}u=
\Re\operatorname{IFFT}_2[(2\pi i f_x)\widehat u],\qquad
\partial_y^{\rm spec}u=
\Re\operatorname{IFFT}_2[(2\pi i f_y)\widehat u],
\]
where \(f_x=\operatorname{fftfreq}(W,L/W)\) and
\(f_y=\operatorname{fftfreq}(H,L/H)\) in standard FFT ordering and are broadcast
along their respective axes.  At \(N=64,L=1\), each frequency vector is
\((0,1,\ldots,31,-32,-31,\ldots,-1)\).  The tangent calculation applies no
finite-difference stencil, Fourier-mode truncation, filtering, or de-aliasing.  A
differentiable JVP is computed with graph construction enabled, and the tangent
mean square is divided by \(\lVert\delta\rVert_2^2+10^{-6}\).
Augmentation+tangent draws its supervised augmentation independently.
