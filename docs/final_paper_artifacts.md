...
runs/figures/2d_galilean_n64_fixed_stress_diagnostics.{png,pdf}
runs/figures/2d_galilean_n64_equivariance_residuals.{png,pdf}
```

The qualitative and companion diagnostic figures are generated from the real
held-out N64 fields and the seed-23 batch-forward-evaluation-matched augmentation
and fixed-weight Aug.+LOCO checkpoints. They use the fixed axis-aligned relative
Galilean stress $\delta=(0.50,0)$, twice the componentwise training radius, and the
shifted target is the exact periodic output action applied to the cached target.
The displayed seed-23, zero-based test index 97 is illustrative: it is
algorithmically selected by the disclosed, outcome-conditioned rule that chooses
the paired fixed-stress improvement nearest the 256-case median. The YAML
specification fixes the checkpoints and stress; the adjacent JSON records the
selection rule, selected index, field/error scales, full-test fixed-stress means,
dataset/checkpoint hashes, plotting-script hash, and diagnostics:
...
For the selected index, relative L2 decreases from 0.4885 to 0.3099 ID and
from 0.5190 to 0.3920 under fixed stress; 59.7% of stressed pixels have lower
absolute prediction error. The fixed-stress diagnostic places that example in
the full seed-23 test distribution: 97.7% of 256 paired gains are positive, with
mean 0.1253 and 10th/50th/90th percentiles 0.0527/0.1267/0.1932; the selected gain
is at empirical CDF 0.504. Full-test seed-23 mean relative L2 is 0.4750 versus
0.3517 ID and 0.6815 versus 0.5563 under fixed stress. The diagnostic also shows
lower Aug.+LOCO mean absolute error in all ten target-gradient bins (0.3669 versus
0.4725 in the lowest-gradient bin and 0.4906 versus 0.5727 in the highest).

The equivariance diagnostic shows the actual per-pixel residual
$|G(Ta)-T G(a)|$ and its signed reduction, with an upper-quartile target-gradient
contour for spatial context. Selected relative defect decreases from 0.5272 to
0.3868, with lower residual on 64.9% of pixels; the full-test seed-23 fixed-stress
mean decreases from 0.7157 to 0.5762. These panels diagnose one disclosed fixed
axis. The independently randomized five-seed OOD-severity evaluation remains the
quantitative evidence for the OOD claim.
