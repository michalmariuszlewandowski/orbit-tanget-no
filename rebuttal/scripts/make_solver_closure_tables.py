...
from otno.data.solvers1d import solve_burgers_1d
from otno.data.solvers2d import solve_navier_stokes_vorticity_2d
from otno.symmetry.registry import build_transform
from otno.symmetry.transforms import Burgers1DGalilean, NavierStokes2DGalilean, TransformSample
...
def _find_galilean_transform(transform: Any) -> Burgers1DGalilean | NavierStokes2DGalilean:
    if isinstance(transform, (Burgers1DGalilean, NavierStokes2DGalilean)):
        return transform
...
def run_closure(config: dict[str, Any], args: argparse.Namespace) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    transform = _find_galilean_transform(build_transform(config.get("symmetry")))
    max_boost = float(args.max_boost if args.max_boost is not None else transform.max_boost)
    boost_dim = 1 if isinstance(transform, Burgers1DGalilean) else 2
    boosts = _sample_boosts(a.shape[0], max_boost, int(args.seed), a.dtype, boost_dim)
    ...
    for start in range(0, a.shape[0], batch_size):
        ...
        sample = TransformSample(
            ...
            epsilon=boost_b.abs().clamp_min(1e-12)
            if isinstance(transform, Burgers1DGalilean)
            else torch.linalg.norm(boost_b, dim=-1).clamp_min(1e-12),
            name=transform.name,
        )
        transformed_input = transform.apply_input(a_b, sample)
        if isinstance(transform, Burgers1DGalilean):
            solved_transformed = solve_burgers_1d(
                transformed_input[..., transform.channel],
                viscosity=viscosity,
        ...
        for local_idx, value in enumerate(residual.detach().cpu()):
            ...
            if isinstance(transform, Burgers1DGalilean):
                row.update(
                    {
                        "delta_boost": float(delta),
                        "delta_boost_norm": float(delta.abs()),
                    }
                )
            else:
